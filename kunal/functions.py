...
def import_root(fname,drop_na = True):
    '''
    Import root file into pandas DataFrame
    '''
    file = uproot.open(fname)

    if 'Bs2KKmumu' in fname:
        ...
        # col_names = [col.decode("utf-8") for col in decay_tree.keys()]
        # df = pd.DataFrame(decay_tree.arrays())
...
def DOCA(df,mu_plus_point,mu_minus_point,bs_point,bs_dir,mu_plus_dir,mu_minus_dir):
    '''
    Analytic calculatiion of Distance Of Closest Approach (DOCA)
    '''
...
def FD(df,tau_start):
    '''
    Analytical calculation of Flight Distance (FD)
    '''
...
def IP(df,bs_point,mu_plus_point,mu_minus_point,mu_plus_dir,mu_minus_dir):
    '''
    Analytic calculation of Impact Parameter (IP)
    '''
...
def root_to_df(fname,drop_na = True):
    '''
    Import and analyse 
    '''
    ...
    if 'Bs2KKmumu' in fname:
        ...
        DOCA(df,mu_plus_point,mu_minus_point,bs_point_2,bs_dir,mu_plus_dir,mu_minus_dir)
        FD(df, df[['phi_ENDVERTEX_X','phi_ENDVERTEX_Y','phi_ENDVERTEX_Z']].to_numpy())
        IP(df,bs_point_2,mu_plus_point,mu_minus_point,mu_plus_dir,mu_minus_dir)
        ...
        DOCA(df,mu_plus_point,mu_minus_point,bs_point,bs_dir,mu_plus_dir,mu_minus_dir)
        FD(df, bs_point)
        IP(df,bs_point,mu_plus_point,mu_minus_point,mu_plus_dir,mu_minus_dir)

    np.savetxt('/home/hep/kj3417/code/columns/{}_columns.txt'.format(fname.split('.')[0].split('/')[-1]),df.columns.to_numpy(),fmt='%s')
